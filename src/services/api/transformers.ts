...
export const normalizeConfigResponse = (raw: unknown): Config => {
    ...
    const sessionAffinity = isRecord(routing)
                            ?
                            normalizeBoolean(routing['session-affinity'] ??
    ...
    const autoRefreshRaw = raw['auto-refresh-interval'] ?? raw.autoRefreshInterval
    if (autoRefreshRaw !== undefined && autoRefreshRaw !== null) {
        const parsed = Number(autoRefreshRaw)
        if (Number.isFinite(parsed) && parsed >= 0) {
            config.autoRefreshInterval = parsed
        }
    }
