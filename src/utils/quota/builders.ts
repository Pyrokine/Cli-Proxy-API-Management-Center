...
export function buildGeminiCliQuotaBuckets(buckets: GeminiCliParsedBucket[]): GeminiCliQuotaBucketState[] {
    ...
    buckets.forEach((bucket) => {
        if (isIgnoredGeminiCliModel(bucket.modelId)) {
            return
        }
        ...
        existing.fallbackRemainingAmount   =
            minNullableNumber(existing.fallbackRemainingAmount, bucket.remainingAmount)
...
export function buildAntigravityQuotaGroups(models: AntigravityModelsPayload): AntigravityQuotaGroup[] {
    ...
    const definitions                     = new Map(ANTIGRAVITY_QUOTA_GROUPS.map((definition) => [
        definition.id,
        definition,
    ] as const))
...
function kimiResetHint(data: Record<string, unknown>): string | undefined {
    ...
    for (const key of absoluteKeys) {
        ...
        if (typeof raw === 'string' && raw.trim()) {
            try {
                const truncated = raw.replace(/(?<frac>\.\d{6})\d+/, '$<frac>')
    ...
    if (Array.isArray(limits)) {
        limits.forEach((item, idx) => {
            const detail        = (item.detail && typeof item.detail === 'object' ? item.detail : item) as
                | KimiUsageDetail
                | KimiLimitItem
            const window        = (item.window && typeof item.window === 'object' ?
                                   item.window :
                {}) as KimiLimitWindow
