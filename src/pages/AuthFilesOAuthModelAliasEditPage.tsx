...
export function AuthFilesOAuthModelAliasEditPage() {
    ...
    const baselineMappings = useMemo(() => {
        if (!resolvedProviderKey) {
            return [buildEmptyMappingEntry()]
        }
        const existing = editor.modelAlias[resolvedProviderKey] ?? []
        return normalizeMappingEntries(existing)
    }, [editor.modelAlias, resolvedProviderKey])

    const effectiveMappings = useMemo(() => {
        if (mappingsOverride?.providerKey === resolvedProviderKey) {
            return mappingsOverride.value
        }
        return baselineMappings
    }, [baselineMappings, mappingsOverride, resolvedProviderKey])
    ...
    const updateMappingEntry = useCallback(
        (index: number, field: keyof OAuthModelAliasEntry, value: string | boolean) => {
            setMappingsOverride((prev) => {
                const base = prev?.providerKey === resolvedProviderKey ? prev.value : baselineMappings
                return {
                    providerKey: resolvedProviderKey,
                    value: base.map((entry, idx) => (idx === index ? { ...entry, [field]: value } : entry)),
                }
            })
        },
    ...
    const addMappingEntry = useCallback(() => {
        setMappingsOverride((prev) => {
            const base = prev?.providerKey === resolvedProviderKey ? prev.value : baselineMappings
            return { providerKey: resolvedProviderKey, value: [...base, buildEmptyMappingEntry()] }
        })
    }, [baselineMappings, resolvedProviderKey])

    const removeMappingEntry = useCallback(
        (index: number) => {
            setMappingsOverride((prev) => {
                const base = prev?.providerKey === resolvedProviderKey ? prev.value : baselineMappings
                ...
                return {
                    providerKey: resolvedProviderKey,
                    value: next.length ? next : [buildEmptyMappingEntry()],
                }
            })
        },
        [baselineMappings, resolvedProviderKey]
    ...
    }, [editor, effectiveMappings, provider, t])
