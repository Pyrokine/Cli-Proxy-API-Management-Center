...
const HTTP_METHOD_REGEX = new RegExp(`\\b(?<method>${HTTP_METHODS.join('|')})\\b`)
...
const HTTP_STATUS_PATTERNS: RegExp[] = [
    /\|\s*(?<code>[1-5]\d{2})\s*\|/,
    /\b(?<code>[1-5]\d{2})\s*-/,
    new RegExp(`\\b(?:${HTTP_METHODS.join('|')})\\s+\\S+\\s+(?<code>[1-5]\\d{2})\\b`),
    /\b(?:status|code|http)[:\s]+(?<code>[1-5]\d{2})\b/i,
    new RegExp(`\\b(?<code>[1-5]\\d{2})\\s+(?:${HTTP_STATUS_TEXTS.join('|')})\\b`, 'i'),
]
...
const extractIp = (text: string): string | undefined => {
    ...
    if (LOG_TIME_OF_DAY_REGEX.test(candidate)) {
        return undefined
    }

    // If no compression marker is present, a valid IPv6 address must contain 8 hextets.
    if (!candidate.includes('::') && candidate.split(':').length !== 8) {
        return undefined
    }
...
const inferLogLevel = (line: string): LogLevel | undefined => {
    ...
    if (/\bfatal\b/.test(lowered)) {
        return 'fatal'
    }
    if (/\berror\b/.test(lowered)) {
        return 'error'
    }
    if (/\bwarn(?:ing)?\b/.test(lowered) || line.includes('警告')) {
        return 'warn'
    }
    if (/\binfo\b/.test(lowered)) {
        return 'info'
    }
    if (/\bdebug\b/.test(lowered)) {
        return 'debug'
    }
    if (/\btrace\b/.test(lowered)) {
        return 'trace'
    }
    return undefined
}
...
export const parseLogLine = (raw: string): ParsedLogLine => {
    ...
    if (tsMatch) {
        timestamp = tsMatch.groups!.ts
        remaining = remaining.slice(tsMatch[0].length).trim()
    }

    let requestId: string | undefined
    const requestIdMatch = remaining.match(/^\[(?<id>[a-f0-9]{8}|--------)]\s*/i)
    if (requestIdMatch) {
        const id = requestIdMatch.groups!.id
        if (!/^-+$/.test(id)) {
            requestId = id
        }
    ...
    if (remaining.includes('|')) {
        ...
        if (statusIndex >= 0) {
            const match = segments[statusIndex].match(/^(?<code>\d{3})$/)
            if (match?.groups) {
                const code = Number.parseInt(match.groups.code, 10)
                if (code >= 100 && code <= 599) {
                    statusCode = code
