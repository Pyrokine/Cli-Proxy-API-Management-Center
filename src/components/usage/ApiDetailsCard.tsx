...
import {type ApiStats, formatCompactNumber, formatUsd} from '@/utils/usage'
import {useMemo, useState} from 'react'
import {useTranslation} from 'react-i18next'

interface ApiDetailsCardProps {
    apiStats: ApiStats[];
    loading: boolean;
    hasPrices: boolean;
}
                          ...
                          <span className={styles.apiBadge}>
                            ...
                            {t('usage_stats.tokens_count')}: {formatCompactNumber(api.totalTokens)}
                              ...
                              <span className={styles.requestCountCell}>
                                ...
                                <span className={styles.requestBreakdown}>
                                  (<span
                                    className={styles.statSuccess}>{stats.successCount.toLocaleString()}</span>{' '}
