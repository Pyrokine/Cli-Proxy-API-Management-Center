...
const resolveAntigravityProjectId = async (file: AuthFileItem): Promise<string> => {
    try {
        ...
        const web          =
                  parsed.web && typeof parsed.web === 'object'
...
const resolveGeminiCliTierLabel = (
    ...
    const rawId                                             = normalizeStringValue(paidTier?.id) ??
                                                              normalizeStringValue(currentTier?.id)
    if (!rawId) {
        return null
    }
...
const fetchClaudeQuota = async (
    ...
    const requestOpts                  = { header: { ...CLAUDE_REQUEST_HEADERS }, authIndex, method: 'GET' as const }
...
const renderClaudeItems = (
    ...
    if (extraUsage && extraUsage.is_enabled) {
        const usedLabel = `$${(extraUsage.used_credits / 100).toFixed(2)} / $${(extraUsage.monthly_limit / 100).toFixed(
            2)}`
        nodes.push(
            h(
                'div',
