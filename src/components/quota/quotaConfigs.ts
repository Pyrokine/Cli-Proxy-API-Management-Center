...
const resolveAntigravityProjectId = async (file: AuthFileItem): Promise<string> => {
    try {
        ...
        const web          = parsed.web && typeof parsed.web === 'object' ?
...
const fetchAntigravityQuota = async (file: AuthFileItem, t: TFunction): Promise<AntigravityQuotaGroup[]> => {
    ...
    let priorityStatus: number | undefined = undefined
...
const fetchGeminiCliQuota = async (file: AuthFileItem, t: TFunction): Promise<GeminiCliQuotaBucketState[]> => {
    ...
    const parsedBuckets = buckets
        .map((bucket) => {
            ...
            const remainingFractionRaw          = normalizeQuotaFraction(bucket.remainingFraction ??
                                                                         bucket.remaining_fraction)
            const remainingAmount               = normalizeNumberValue(bucket.remainingAmount ??
                                                                       bucket.remaining_amount)
            const resetTime                     = normalizeStringValue(bucket.resetTime ?? bucket.reset_time) ??
                                                  undefined
...
/** Render usage-window quota rows (shared by Codex and Claude) */
const renderUsageWindowRows = (
    ...
    QuotaProgressBar: QuotaRenderHelpers['QuotaProgressBar'],
): ReactNode[] =>
    windows.map((window) => {
...
const renderCodexItems = (quota: CodexQuotaState, t: TFunction, helpers: QuotaRenderHelpers): ReactNode => {
    ...
    nodes.push(...renderUsageWindowRows(windows, t, styleMap, QuotaProgressBar))
...
const renderGeminiCliItems = (quota: GeminiCliQuotaState, t: TFunction, helpers: QuotaRenderHelpers): ReactNode => {
    ...
    return buckets.map((bucket) => {
        ...
        const titleBase            = bucket.modelIds && bucket.modelIds.length > 0 ?
...
const renderClaudeItems = (quota: ClaudeQuotaState, t: TFunction, helpers: QuotaRenderHelpers): ReactNode => {
    ...
    if (extraUsage && extraUsage.is_enabled) {
        ...
        const limit     = (extraUsage.monthly_limit / 100).toFixed(2)
        const usedLabel = `$${used} / $${limit}`
        nodes.push(
            h(
                'div',
